...
import { BttvEmote, FfzEmote, SevenTvEmote } from 'src/types/emotes';
...
export class EmoteFetcher {
  ...
  async fetchBttvGlobalEmotes(): Promise<BttvEmote[]> {
    const response = await this._client.get<BttvApiEmote[]>('https://api.betterttv.net/3/cached/emotes/global');
  ...
  async fetchFfzGlobalEmotes(): Promise<FfzEmote[]> {
    ...
    const response = await this._client.get<FfzApiEmoteSetResponse>('https://api.frankerfacez.com/v1/set/3');
  ...
  async fetch7tvGlobalEmotes(): Promise<SevenTvEmote[]> {
    const response = await this._client.get<SevenTvApiEmote[]>('https://api.7tv.app/v2/emotes/global');
